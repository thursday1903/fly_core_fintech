...
/**
 * Thread Pool dung execute cac thread handle cua Http Server.
 *
 * @author VietTung
 */
public class ServerHttpThreadPool {

	static ThreadPoolExecutor executor = null;

	@SuppressWarnings({ "rawtypes", "unchecked" })
	public ServerHttpThreadPool() {
	...
	private static ServerHttpThreadPool instance = null;

	public static ServerHttpThreadPool GetInstance() {
		if (instance == null)
			new ServerHttpThreadPool();
		return instance;
	}
	...
	public static void main(String[] args) {
		for (int i = 0; i < 100; i++) {
			...
			ServerHttpThreadPool.GetInstance().execute(new Thread(new Runnable() {
		...
		System.out.println(ServerHttpThreadPool.GetInstance().getThreadPoolInfo2());
		while (ServerHttpThreadPool.GetInstance().getThreadExecutor().getActiveCount() > 0) {
		}
		// System.out.println(HttpThreadPool.GetInstance().getThreadPoolInfo2());
		// System.out.println(HttpThreadPool.GetInstance().getThreadPoolInfo2());

		ServerHttpThreadPool.GetInstance().stop();
	}
